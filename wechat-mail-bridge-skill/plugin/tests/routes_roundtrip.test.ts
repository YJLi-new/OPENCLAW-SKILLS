...
describe("route roundtrip flow", () => {
  ...
  it("processes event -> claim -> ack -> receipts/jobs visibility", async () => {
    const secret = `secret_${randomUUID()}`;
    sqlitePath = path.join(os.tmpdir(), `wxmail_roundtrip_${randomUUID()}.db`);
    ...
    expect(Boolean(ingestBody.result.jobId)).toBe(true);
    expect(Boolean(ingestBody.result.commandId)).toBe(true);
    ...
    expect(claimBody.commands.length).toBe(1);
    expect(claimBody.commands[0].commandId).toBe(ingestBody.result.commandId);
    ...
    const receipts = await app.inject({
      method: "POST",
      url: "/api/v1/admin/receipts/list",
      headers: authHeaders(secret),
      payload: { limit: 10 }
    });
    expect(receipts.statusCode).toBe(200);
    const receiptsBody = receipts.json() as {
      ok: boolean;
      receipts: Array<{ commandId: string; status: string }>;
    };
    expect(receiptsBody.ok).toBe(true);
    expect(receiptsBody.receipts.some((r) => r.commandId === ingestBody.result.commandId && r.status === "sent")).toBe(
      true
    );
